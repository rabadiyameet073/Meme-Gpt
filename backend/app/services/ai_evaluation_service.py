...
FAILURE_ANALYSIS_CHECKLIST = [
    {"step": 1, "action": "Check embedding model", "detail": "Did sentence-transformers model weights update break compatibility?"},
    {"step": 2, "action": "Check Qdrant index", "detail": "Is the collection index corrupted, degraded, or missing vector points?"},
    {"step": 3, "action": "Check Groq response quality", "detail": "Is the LLM returning valid schema-compliant JSON payloads?"},
    {"step": 4, "action": "Check new memes", "detail": "Did recent indexing batches introduce low-quality or mislabeled memes?"},
    {"step": 5, "action": "Check score distribution", "detail": "Are cosine similarity scores clustered unusually low or high?"},
]


def get_evaluation_metrics_catalog() -> Dict[str, Any]:
    """Return all AI quality and engagement metrics."""
    return {
...
def run_offline_search_evaluation(
    ...
    If predictions_by_query is not provided, defaults to simulated predictions on BENCHMARK_TEST_CASES.
    """
    total_p5 = 0.0
    total_r10 = 0.0
    total_mrr = 0.0
    ...
    for test in BENCHMARK_TEST_CASES:
        query = test["query"]
        relevant = test["relevant_memes"]
        ...
        if predictions_by_query and query in predictions_by_query:
            ...
            predicted = relevant[:2] + test["irrelevant_memes"][:1] + relevant[2:]
    ...
    n = max(1, len(BENCHMARK_TEST_CASES))
